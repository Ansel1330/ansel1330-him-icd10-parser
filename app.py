...
# We assign specific hex colors for custom HTML components depending on selection
if st.session_state.dark_mode:
    ...
    sub_text = "#94A3B8"       # Muted slate gray
else:
    ...
    sub_text = "#475569"       # Muted dark slate

# Inject high-contrast styling onto custom HTML containers safely
st.markdown(f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
    
    /* Apply clean font family globally without breaking system text paths */
    html, body, [class*="st-"] {{
        font-family: 'Inter', sans-serif !important;
    }}
    
    /* Custom Scoped Classes to prevent text blinding */
    .main-title {{ 
        font-size: 34px; 
        font-weight: 700; 
    ...
    /* Target the sidebar collapse/expand toggle elements to visually guide users */
...
# --- 3. SIDEBAR HEADER & NAVIGATION BRANDING ---
...
# Replaced the confusion layout header with an explicit navigation instruction map label
...
# --- 7. HUB MODULE 3: LEADERBOARD RANKING POOL ---
    ...
    if len(st.session_state.competition_pool) == 0:
        ...
        with col_board:
            st.markdown(f"### Active Standing Board", unsafe_allow_html=True)
            st.dataframe(df_pool, use_container_width=True)
            
        with col_chart:
            st.markdown(f"### Class Ranking Visual Spread", unsafe_allow_html=True)
